...
10 quater. **La tira del pedido en curso** (fase 12). Solo en el móvil, entre el grid y la barra del pedido, en el hueco que ya sobraba. Enseña las líneas del pedido actual **en el mismo orden que la hoja** —la más reciente la última, pegada a la barra—, cada una en una fila de 44 px: la cantidad delante si es mayor que 1 («2 ×»), el nombre, los extras detrás en `--ink-3` («· avena») cortados con elipsis, y una **«×» de 44 × 44** con `aria-label` «Quitar Latte con avena del pedido» que quita una unidad (o la línea entera si queda una). Tocar el nombre convierte esa línea en la bebida actual de la fila de extras; se marca con fondo `--surface-2`, nunca con una franja lateral. **Con el pedido vacío no se dibuja.** Cada «×» deja «Quitada 1 Latte · Deshacer» 8 s, que devuelve la línea entera —se guarda la línea, no su id—; el aviso **se retira al servir**, porque su «Deshacer» apuntaría a un pedido que ya no existe, y **sube por encima de la tira** para no caer sobre ninguna «×» (decisión 98 una fila más arriba).
   - **Cuántas filas se ven lo mide el navegador**: el hueco real entre el borde inferior de la última bebida y el final del grid, en ranuras de 45 px (44 + la línea que separa), **mínimo 1 y máximo 4**. Con más líneas de las que caben, la lista se desplaza dentro de sí misma —nunca la página— y se va sola a la más reciente; encima, un rótulo de 15 px **«+N más · ver todo»** que abre la hoja del pedido y que **ocupa una ranura**, así que con una sola no sale (ahí la cuenta entera la da «Pedido actual (N)» de la barra de abajo). Medido con las catorce bebidas: **2 ranuras a 402 × 874** (hueco 103 px, tira de 89 px con dos líneas, 7 px de holgura) y **1 a 402 × 781** (hueco 62 px, tira de 44 px, 12 px de holgura). El grid **no se mueve**: la última bebida queda en el mismo píxel con la tira puesta y escondida (687 y 635 px), sin scroll de grid ni de página.
   ...
   - En el **iPad no existe**: el pedido ya está entero en su columna de 360 px.
