...
10 bis. **En el móvil, con el pedido actual vacío, la barra dice «Pedido actual (0)»** y el último pedido servido lo cuenta la tira, justo encima (regla 10 quater, fase 13): decirlo en los dos sitios, a 44 px de distancia, es decirlo una vez de más. El texto de la fase 11 —«Último · 12:41 · Latte · avena, 2 × Cortado», 16 px, una línea con elipsis, la hora en `--ink-3`, y tocarlo abre la hoja con ese pedido ya desplegado— **se queda como recurso**: sale cuando la tira no lo cuenta, es decir en una pantalla donde no quepa ni con su rótulo y una fila. Sin nada servido, «Sin pedidos todavía». En cuanto el pedido tiene una línea vuelve «Pedido actual (N)»; en pausa, corrigiendo y en modo Rápido mandan sus etiquetas.
...
10 quater. **La tira, con sus dos estados** (fases 12 y 13). Solo en el móvil, entre el grid y la barra del pedido, en el hueco que ya sobraba. **Se nombra a sí misma siempre**: un rótulo de 15 px en `--ink-3` encima de la lista, en una caja de 14 px —«Pedido actual» o «Ya servidos»—, en el mismo sitio y del mismo tamaño en los dos estados; lo que cambia es la palabra. El color acompaña y no manda: el estado vivo es tarjeta blanca (`--surface`) con el texto en tinta, el de servidos no es tarjeta —fondo `--bg` con un filo de 1 px en `box-shadow: inset`, que no suma alto— y el texto en `--ink-2`.

   - **Estado «Pedido actual»**: enseña las líneas del pedido actual **en el mismo orden que la hoja** —la más reciente la última, pegada a la barra—, cada una en una fila de 44 px: la cantidad delante si es mayor que 1 («2 ×»), el nombre, los extras detrás en `--ink-3` («· avena») cortados con elipsis, y una **«×» de 44 × 44** con `aria-label` «Quitar Latte con avena del pedido» que quita una unidad (o la línea entera si queda una). Tocar el nombre convierte esa línea en la bebida actual de la fila de extras; se marca con fondo `--surface-2`, nunca con una franja lateral. Cada «×» deja «Quitada 1 Latte · Deshacer» 8 s, que devuelve la línea entera —se guarda la línea, no su id—; el aviso **se retira al servir**, porque su «Deshacer» apuntaría a un pedido que ya no existe, y **sube por encima de la tira** para no caer sobre ninguna «×» (decisión 98 una fila más arriba).
   - **Estado «Ya servidos»** (fase 13): con el pedido actual vacío y al menos un pedido servido no anulado, la tira enseña **los pedidos ya servidos**, el más reciente el último, pegado a la barra. Cada fila de 44 px: la **hora** en tabular `--ink-3` («12:41») y la frase del pedido detrás en `--ink-2` («Latte · avena, 2 × Cortado»), cortada con elipsis. **Sin «×»**: anular un pedido servido cambia el contador, el medidor y el coste, y se queda donde está —en la hoja, con su motivo y su «Deshacer»—. Tocar una fila abre la hoja del pedido **con ese pedido desplegado** en «Últimos pedidos» (`aria-label` «Ver el pedido de las 12:41: Latte · avena»). Con el pedido vacío y **nada servido**, la tira no se dibuja. En **modo Rápido** tampoco, de momento: apuntado como propuesta en `UX-REVISION-2`. **Con el servicio en pausa sí se dibuja**: es una lectura, no un registro, y repasar lo servido entre dos turnos es justo lo que se hace en una pausa.
   - **Cuántas filas se ven lo mide el navegador**: el hueco real entre el borde inferior de la última bebida y el final del grid, **menos los 14 px del rótulo**, en ranuras de 45 px (44 + la línea que separa), **mínimo 1 y máximo 4**. Con más de las que caben, «Pedido actual» se desplaza dentro de sí mismo —nunca la página— y se va solo a la más reciente; encima, un rótulo de 15 px **«+N más · ver todo»** que abre la hoja del pedido y que **ocupa una ranura**, así que con una sola no sale (ahí la cuenta entera la da «Pedido actual (N)» de la barra de abajo). La tira **no paga hueco de columna**: un margen negativo de `--work-gap` lo cancela y su rótulo hace de separación. Por debajo de 58 px (rótulo + una fila) no se dibuja y la barra de abajo recupera el texto del último pedido.
   - **Lo medido con las catorce bebidas** (`npm run capturas:servidos`). El presupuesto no es el mismo en los dos estados: con el pedido vacío el bloque de extras se repliega y deja 108 px libres.

     | | 402 × 874 | 402 × 781 |
     |---|---|---|
     | Presupuesto en «Pedido actual» | 103 px | 62 px |
     | Presupuesto en «Ya servidos» | 211 px | 170 px |
     | «Pedido actual» con 2 líneas | 103 px de tira (2 filas) · holgura 0 | 58 px (1 fila) · holgura 4 |
     | «Ya servidos» con 3 pedidos | 148 px (3 filas) · holgura 63 | 148 px (3 filas) · holgura 22 |
     | «Ya servidos» lleno | 192 px (3 filas + «+4 más») · holgura 19 | 147 px (2 filas + «+5 más») · holgura 23 |

     El grid **no se mueve** en ninguno de los casos: la última bebida queda en el mismo píxel con la tira puesta, escondida y en los dos estados (687 y 635 px montando, 579 y 527 con el pedido vacío), con `scrollHeight === clientHeight` en el grid y en la página.
   - Para que entre, el corte de las pantallas bajas sube de 750 a **820 px** de alto (tile de 72 px y hueco de 6 en el iPhone instalado) y el grid deja de reservar la cola de desplazamiento de 8 px en el móvil, donde no se desplaza nunca. Por tamaño de pantalla y **no** por si la tira está puesta: un grid que encoge al tocar la primera bebida mueve los objetivos debajo del dedo.
   - En el **iPad no existe**, en ninguno de los dos estados: el pedido y «Últimos pedidos» ya están enteros en su columna de 360 px.
10 quinquies. **Salir de una hoja** (fase 12). Vale para las tres —la lateral, la de abajo y la del pedido—. La **cabecera va pegada arriba** (`position: sticky`) con su fondo, el título y la **«X» de ≥ 44 px siempre a la vista**, se haya desplazado lo que se haya desplazado: abrir el Resumen por el ancla de «Pedidos» lo deja desplazado 724-817 px y antes el título y la salida quedaban fuera de la pantalla. El ancla reserva el alto de la cabecera; la línea inferior aparece solo con contenido desplazado debajo y va en `box-shadow`, que no mueve la hoja. **En el móvil se cierra deslizando**: arrastrar hacia abajo más de 80 px cierra, menos vuelve a su sitio; el gesto solo arranca cuando nada entre lo que se toca y la hoja está desplazado —con la lista a medias, el dedo desplaza— y desde el **asa** (4 × 36 px centrada arriba, dentro de la cabecera y en absoluto) o la cabecera se arrastra siempre. Con `prefers-reduced-motion` la hoja no sigue al dedo: se cierra al soltar. En el iPad no hay gesto ni asa. `Escape`, tocar el fondo y el foco devuelto no cambian. **Nunca una ventana emergente centrada**: `DESIGN.md` las prohíbe en el flujo de servir.
