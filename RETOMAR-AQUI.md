...
**Última sesión**: 12/09/2026 (fase 12: el pedido a la vista y salir de la hoja) · **Evento real**: boda del 12/09/2026 (Nicolas la usa en su **iPhone 17 Pro**, no lleva iPad)
...
| `docs/DECISIONES.md` | **122** decisiones fechadas. Respetarlas o discutirlas, no ignorarlas. Las 110-122 son las de hoy |
...
- **Cuántas filas se ven lo mide el navegador** (`src/ui/tira.tsx` + el efecto de `barra.tsx`): ranuras de 45 px sobre el hueco real. Hoy son **2 filas a 402 × 874 y 1 a 402 × 781**. Con más líneas, la lista se desplaza dentro de sí misma y encima sale «+N más · ver todo», que ocupa una ranura (con una sola no sale).
...
**Límite conocido, y lo que se gastó para la tira**: con las **14 bebidas** de hoy quedan **12 px de holgura a 402 × 781** y 7 a 402 × 874, ya con la tira puesta. Para que cupiera, el corte de las pantallas bajas subió de 750 a **820 px** —así que el iPhone instalado va con **tiles de 72 px**, el primer recurso que estaba previsto— y el grid dejó de reservar 8 px de cola de desplazamiento en el móvil. **No ampliar la carta sin volver a medir con `npm run capturas:pedido`**: el siguiente recurso ya no es el tile, es la carta.
