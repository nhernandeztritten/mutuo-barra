...
- **La tira, con sus dos estados** (fases 12 y 13), entre el grid y la barra del pedido. **Se nombra a sí misma siempre**, en el mismo sitio y del mismo tamaño: un rótulo de 15 px en `--ink-3`, caja de 14 px, que dice «Pedido actual» o «Ya servidos». Lo que cambia entre estados es la palabra; **el color acompaña, no manda**.
  - **«Pedido actual»**: las líneas del pedido en el orden de la hoja —la más reciente la última—, en filas de 44 px: la cantidad delante si es mayor que 1, el nombre, los extras en `--ink-3` y una **«×» de 44 × 44** que quita esa línea con su «Deshacer» de 8 s. Tocar el nombre la hace la bebida actual, con fondo `--surface-2` y nunca con franja lateral. Es lo vivo: tarjeta `--surface`, texto `--ink`.
  - **«Ya servidos»**: con el pedido vacío, los pedidos ya servidos, el más reciente pegado a la barra. Hora en tabular `--ink-3` delante y la frase del pedido en `--ink-2` detrás, con elipsis. **Sin «×»** —anular es destructivo y vive en la hoja—; tocar la fila abre ese pedido desplegado. Es historia: **no es una tarjeta**, fondo `--bg` con un filo de 1 px en `box-shadow: inset` (un borde de verdad sumaría 2 px al alto).
  - Vacía y sin nada servido, **no se dibuja**. Cuántas filas se ven **lo mide el navegador**: el hueco real entre la última bebida y el final del grid, menos los 14 px del rótulo, en ranuras de 45 px, mínimo 1 y máximo 4; con más líneas, la lista se desplaza dentro de sí misma y encima va «+N más · ver todo», que ocupa una ranura. La tira **no paga hueco de columna**: su rótulo hace de separación. Medido: «Pedido actual» **2 filas a 402 × 874 y 1 a 402 × 781**; «Ya servidos», **3 filas** en los dos, porque con el pedido vacío el bloque de extras se repliega y deja 108 px. El grid, quieto en el mismo píxel. Para que entre, **el tile baja a 72 px por debajo de 820 px de alto** —antes 750— y el grid deja de reservar la cola de desplazamiento de 8 px en el móvil. Por tamaño de pantalla y no por si la tira está puesta: un grid que encoge al tocar la primera bebida mueve los objetivos debajo del dedo.
- **Barra inferior del pedido**: la de siempre, con el botón diciendo la cuenta entera («Servir 3 bebidas»). Con el pedido vacío dice «Pedido actual (0)» y **no repite el último servido**: eso lo cuenta la tira de encima (fase 13). El texto «Último · 12:41 · Latte · avena, 2 × Cortado» —16 px, una línea con elipsis— queda de recurso para una pantalla donde la tira no quepa, y «Sin pedidos todavía» para cuando no hay nada servido. El toast sube por encima de ella: lleva «Deshacer», que es un control de verdad.
...
## Hojas: entrar es fácil, salir tiene que serlo igual
La **cabecera de toda hoja va pegada arriba** (`position: sticky`), con su fondo, el título y la **«X» de ≥ 44 px siempre a la vista**, se haya desplazado lo que se haya desplazado. La línea inferior aparece solo cuando hay contenido desplazado debajo, y va en `box-shadow`: un borde que aparece mueve la hoja un píxel. Una hoja que se abre por un ancla reserva el alto de la cabecera.

En el **móvil** hay un segundo camino de salida: **arrastrar la hoja hacia abajo** más de 80 px la cierra, menos la devuelve a su sitio. El gesto solo arranca cuando no queda nada que desplazar —con la lista a medias, el dedo desplaza—; desde el **asa** o la cabecera se arrastra siempre. El **asa** es una barra de 4 × 36 px centrada arriba, dentro de la cabecera y posicionada en absoluto para no robarle alto a la hoja. Con `prefers-reduced-motion` la hoja no sigue al dedo: se cierra al soltar. En el iPad no hay gesto ni asa: la hoja entra por el lado y hay teclado, ratón y sitio de sobra.
